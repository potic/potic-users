...
docker run -dit --name pocket_square_users --link pocker_square_mongo -v /logs:/logs -e MONGO_PASSWORD=$MONGODB_PASSWORD -p 28101:8080 pocketsquare/pocket-square-users:$TAG
----
